...
de_data <- read.csv("~/Documents/df/de data/all_de")

# Extract article text and convert to tm corpus data structure 
de_articles.text = tm::Corpus(tm::VectorSource(de_data$text)) 
...
# list of other stop words not caught by tm algorithm
domainwords =  c('fã¼','daten','dass','neu','neue','neuen','mehr','müssen','machen','sagt','viel','schon','mai','gibt','sei','etwa','wer','wo','wem','immer','bereit','dabei','beim','an','der','die','das','den','des','dem','sowie','dafür','ganz','dürfen','fall','sollen','allerdings','auf','jeder','jedes','jeden','jedes','jede','zudem','damit','dazu','sollten','sollen','darüber','darauf','wurden','deren','davon','jedoch','viel','gilt','seit','wurde','wurdet','wurdest','würden','würde','würdet','würdest','viele','dsgvo','xaâ','¼ber','nnen','sagte','lã','mã','groã','mã¼ssen','hã','rden','xfen','unserer','geschã','bisher','vielen','unternehmen','datenschutzgrundverordnung','datenschutz','xfe','xft')

# transform all terms to lower case, remove numbers, strip whitespace
# remove stop words and domain words
de_articles.text = de_articles.text %>%
...
# Create DTM
dtm.de_articles = tm::DocumentTermMatrix(de_articles.text)

# remove sparse terms
dtm.sparse = tm::removeSparseTerms(dtm.de_articles, 0.99) # adjust to 0.9, 0.99, 0.9999 ...
...
tf=term_freq %>%
  ...
  ylab('Frequency') +
  ggtitle('DE data: top 20 terms (stemmed)') +
